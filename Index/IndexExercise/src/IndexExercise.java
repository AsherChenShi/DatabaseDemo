...
public class IndexExercise {
    public static void main(String... args) {
        ...
        DataGenerator gen = new DataGenerator(753200);
        String queryString = gen.getRandomString();
        System.out.println("queryTuples:"+queryString);
        System.out.println("================================");
    ...
    public List<Tuple> getEqualStringTuples(String string) {
        ...
        for (Tuple tuple : tuples) {
            if(tuple.string.equals(string)){
                ret.add(tuple);
                //System.out.println("TestString:"+tuple);
            }
        }
        return ret;
    }
...
class DenseIndex extends Index {
	HashMap<String, List<Tuple>> DenseIndexMap = new HashMap<>();
    DenseIndex(ArrayList<Tuple> tuples) {
        super(tuples);
    	for(Tuple tuple : tuples) {
    		if(DenseIndexMap.containsKey(tuple.string)) {
    			DenseIndexMap.get(tuple.string).add(tuple);
    		}else {
    			List<Tuple> DenseIndexTuples = new ArrayList<>();
        		DenseIndexTuples.add(tuple);
        		DenseIndexMap.put(tuple.string,DenseIndexTuples);
    		}   		
    	}
    }
    ...
    @Override
    public List<Tuple> getEqualStringTuples(String string) {
    	List<Tuple> equ = new ArrayList<>();
    	Iterator<Map.Entry<String, List<Tuple>>> itr = DenseIndexMap.entrySet().iterator();
    	while(itr.hasNext()) {
    		Map.Entry<String, List<Tuple>> entry = itr.next(); 
        	if(entry.getKey().equals(string)){
        		for(int i = 0; i < entry.getValue().size(); i++) {
        			equ.add(entry.getValue().get(i));
        		}
            	//System.out.println("equalString:"+entry.getKey());
            	//System.out.println("equalTuple:"+equ);
            }
        }
        return equ;
    }
    ...
    @Override
    public List<Tuple> getGreaterEqualsStringTuples(String string) {
    	List<Tuple> Gre = new ArrayList<>();
        for (Map.Entry<String, List<Tuple>> mapElement : DenseIndexMap.entrySet()) {
            if(mapElement.getKey().compareTo(string) >= 0){
        		for(int i = 0; i < mapElement.getValue().size(); i++) {
        			Gre.add(mapElement.getValue().get(i));
        		}
            }
        }
        return Gre;
