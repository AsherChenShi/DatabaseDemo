...
public class JoinOrderingExercise {
    ...
    public static void main(String... args) throws IOException {
        ...
        JoinTree plan = DPchainquery(relations);
        System.out.println("DPchainquery: \t" + plan.toString() + "\tCost: " + plan.cost(relations));
    ...
    */
    ...
    static JoinTree DPchainquery(IRelationDirectory directory) {
    	List<Pair<String, Integer>> relations = directory.getRelations();
    	List<String> RelationSequence = new List<>();
    	List<Integer> Start_Point = new List<>();
    	for (Pair<String, Integer> entry : relations) {
    		RelationSequence.add(entry.getKey());
    	}
    	for(int i=0; i<RelationSequence.size(); i=i+2 ) {
    		Relation a = new Relation(RelationSequence.get(i));
    		Relation b = new Relation(RelationSequence.get(i+1));
    		Join intermediateResult = new Join(a,b);
    		Start_Point.add(intermediateResult.cardinality(directory));
    	}
    	int beginCost = 0;
    	int endCost = 0;
    	int middleCost = 0;
    	for(int i=0;i<Start_Point.size();i++) {
    		if(i==0) {    			
    			Relation a = new Relation(RelationSequence.get(0));
        		Relation b = new Relation(RelationSequence.get(1));
        		Join beginTree = new Join(a,b);
        		for(int j=2; j<RelationSequence.size(); j++ ) {
        			Relation c = new Relation(RelationSequence.get(j));
        			Join beginTree = new Join(beginTree,c);
    			}
        		beginCost = beginTree.cardinality(directory);
    		}
    		if(i==Start_Point.size()-1) {
    			Relation a = new Relation(RelationSequence.get(RelationSequence.size()-2));
        		Relation b = new Relation(RelationSequence.get(RelationSequence.size()-1));
        		Join endTree = new Join(a,b);
        		for(int j=RelationSequence.size()-2; j>0; j-- ) {
        			Relation c = new Relation(RelationSequence.get(j));
        			Join beginTree = new Join(endTree,c);
        		}
        		endCost = endTree.cardinality(directory);
    		}
    		Relation a = new Relation(RelationSequence.get(i));
    		Relation b = new Relation(RelationSequence.get(i+1));
    		Join middleTree = new Join(a,b);
    		for(int j=i; j<RelationSequence.size(); j++ ) {//calculate the join of right side
    			Relation c = new Relation(RelationSequence.get(j));
    			Join middleTree = new Join(middleTree,c);
    		}
    		for(int j=i-1; j>0; j-- ) {//calculate the join of left side
    			Relation c = new Relation(RelationSequence.get(j));
    			Join middleTree = new Join(middleTree,c);
    		}
    		middleCost = middleTree.cardinality(directory);	
    	}
    	
    	if(beginCost<endCost && beginCost<middleCost){  
            return beginTree;  
        }else if(endCost<middleCost){  
            return endTree;  
        }else{  
            return middleTree;  
        }  
